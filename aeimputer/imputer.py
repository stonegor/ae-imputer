...
class AEImputer(TransformerMixin):
    """
    ...
    def __init__(
        ...
        n_layers=3,
        hidden_dims=None,
        latent_dim_percentage="auto",
        max_epochs=1000,
        lr=1e-3,
        patience=10,
        min_delta=1e-6,
        max_impute_iters=15,
        preimpute_strategy="mean",
        preimpute_at_train=False,
        device="cpu",
        batch_size=32,
    ):
        ...
        if self.device == "cuda" and not torch.cuda.is_available():
            self.device = "cpu"
            warnings.warn(
                "device = 'cuda' is specified, but no avaliable cuda devices were found. switching to cpu. (torch.cuda.is_available() is False)"
            )
    ...
    def fit(self, X, verbose=False):
        ...
        if self.preimpute_at_train:
            X = self._preimpute(X)
        elif incomplete_rows_mask.all():
            warnings.warn(
                "No complete rows in X, falling back to 'preimpute_at_train' = True"
            )
            X = self._preimpute(X)
        else:
    ...
    def transform(self, X, verbose=False):
        ...
        dataset = TensorDataset(
            torch.tensor(X[incomplete_rows_mask]),
            torch.tensor(nan_mask[incomplete_rows_mask]),
        )
    ...
    def _preimpute(self, X):
        ...
        if self.preimpute_strategy == "noise":
            ...
            raise TypeError(
                f"Expected preimpute_strategy to be in ['noise','mean','median','most_frequent'], got {self.preimpute_strategy} instead"
            )
    ...
    def _build_model(self) -> nn.Module:
        ...
        if self.latent_dim_percentage == "auto":
            ...
            raise TypeError(
                "Expected latent_dim_percentage to be of type float or 'auto'"
            )

        if self.hidden_dims == None:
            self.hidden_dims = list(
                linear_generator(
                    self.in_features, latent_dim, n_steps=self.n_layers + 1
                )
            )[1:]
        ...
        if self.model_type == "vanilla":
            ...
            model = VariationalAutoEncoder(
                self.in_features, self.n_layers, self.hidden_dims
            )
        else:
