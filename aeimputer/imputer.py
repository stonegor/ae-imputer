...
# TODO: Refactor VAEImputer and AEImputer classes to avoid repeating code
...
class AEImputer(_BaseImputer):
    def __init__(self, missing_values = np.nan, n_layers = 3, hidden_dims = None, latent_dim_percentage = 'auto',max_epochs = 1000, lr = 1e-3, patience = 10, min_delta = 1e-6, max_impute_iters = 15, init_nan = 'mean', device = 'cpu', batch_size = 32):
    ...
    def fit(self, X, verbose = False):
        ...
        for epoch in range(self.max_epochs):
            ...
            for data in dataloader:
                ...
                loss = criterion(batch_reconstruction, batch)
    ...
    def transform(self, X, verbose = False):
        ...
        for data in dataloader:
            ...
            for epoch in range(self.max_impute_iters):
                ...
                imputed_batch = self.model(batch)[-1]
...
class VAEImputer(_BaseImputer):
    ...
    def fit(self, X, verbose = False):
        ...
        self.model = VariationalAutoEncoder(self.in_features, self.n_layers, self.hidden_dims).to(self.device)
        optimizer = optim.Adam(self.model.parameters(), lr=self.lr)        
        criterion = nn.MSELoss() 
        ...
        for epoch in range(self.max_epochs):
            ...
            for data in dataloader:
                ...
                mean, log_var, batch_reconstruction = self.model(batch)
                loss = criterion(batch_reconstruction, batch) + kld_loss(mean, log_var)
